...
export async function approveFeeWaiverAction(waiverId: string, notes?: string) {
  ...
  try {
    await db.$transaction(async (tx) => {
      ...
      if (newBalance === 0 && newTotal === 0) newStatus = "WAIVED";
      ...
      if (waiver.studentBillItemId) {
        await tx.studentBillItem.update({
          ...
          data: { waivedAmount: { increment: waiver.calculatedAmount } },
        });
      }

      // Double-entry: Dr Fee Waiver Expense / Cr Accounts Receivable
      const waiverExpense = await findAccountByCode(tx, ctx.schoolId, ACCOUNTS.EXPENSE_FEE_WAIVER);
      const arAccount = await findAccountByCode(tx, ctx.schoolId, ACCOUNTS.FEES_RECEIVABLE);
      let journalId: string | null = null;
      if (waiverExpense && arAccount && toNum(waiver.calculatedAmount) > 0) {
        const posted = await postJournalTransaction(tx, {
          ...
          lines: [
            { accountId: waiverExpense.id, side: "DEBIT", amount: toNum(waiver.calculatedAmount), narration: `Waiver ${waiverId.slice(-8)}` },
            { accountId: arAccount.id, side: "CREDIT", amount: toNum(waiver.calculatedAmount), narration: `Waiver ${waiverId.slice(-8)}` },
          ],
        });
