...
export async function applyScholarshipAction(
  ...
  try {
    studentScholarship = await db.$transaction(async (tx) => {
      const ss = await tx.studentScholarship.create({
        data: {
          schoolId: ctx.schoolId,
          studentId,
          scholarshipId,
      ...
      // Apply scholarship discount to the outstanding bill, then post:
      //   Dr Scholarship Expense / Cr Accounts Receivable
      if (studentBill && appliedAmount > 0) {
        const actualDiscount = Math.min(appliedAmount, toNum(studentBill.balanceAmount));
        if (actualDiscount > 0) {
          await tx.studentBill.update({
            where: { id: studentBill.id },
            data: {
              totalAmount: { decrement: actualDiscount },
              balanceAmount: { decrement: actualDiscount },
            },
          });
        }
        ...
        if (scholarshipExp && arAccount && actualDiscount > 0) {
          const posted = await postJournalTransaction(tx, {
            schoolId: ctx.schoolId,
            date: new Date(),
            description: `Scholarship "${scholarship.name}" applied`,
            referenceType: "Scholarship",
            referenceId: ss.id,
            createdBy: ctx.session.user.id,
            isAutoGenerated: true,
            lines: [
              { accountId: scholarshipExp.id, side: "DEBIT", amount: actualDiscount, narration: scholarship.name },
              { accountId: arAccount.id, side: "CREDIT", amount: actualDiscount, narration: scholarship.name },
            ],
          });
...
export async function removeStudentScholarshipAction(id: string) {
  ...
  try {
    await db.$transaction(async (tx) => {
      if (existing.journalTransactionId) {
        await reverseJournalTransaction(tx, existing.journalTransactionId, {
          schoolId: ctx.schoolId,
          reversedBy: ctx.session.user.id,
          description: `Scholarship "${existing.scholarship.name}" removed from student`,
        });
      }
